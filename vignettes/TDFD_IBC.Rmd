...
# Aims
In this repository, you will find the tutorial reporting the code to reproduce the analyses and figures carried out in Alric *et al.* (2022). The scope of this tutorial is to increase its reproducibility, clarity, transparency and dissemination. Data analysis was done using R version 3.6.3. This docume
nt was compiled with the *rmarkdown* package version 2.8. This tutorial is licensed under CC BY-NC-ND 4.0, which means you are free to share, copy and redistribute this tutorial in any medium or format under the terms of attribution of appropriate credit, non-commercial purposes and no derivatives. The citation is:
...
blo_eltonian <- c(7, 2, 3, 4, 7, 4, 5, 4, 8, 9, 7) # Number of categories by biological trait
...
rownames(abun_subg.1) <- paste(abun_subg$cd_opecont, abun_subg$group, sep = "_")
...
| <span> |
| :------------------------------------------------------------------------------------------------------------ |
| **WARNING:** Please go to the Figure 3 section to see how group-specific taxa (i.e., taxa.Si.G1, taxa.Si.G3 and taxa.Si.common) were estimated |
| <span> |
```{r, eval = FALSE}
# Create name vector for taxa either specifc to UnCont1 and Cont or commun to both
taxa.Si.G1 <- c("Euleuctra", "Perla", "Silo", "Cheumatopsyche", "Lepidostoma", "Limnephilinae" , "Odontocerum", "Polycentropus",
...
taxa.Si.G3 <- c("Protonemura", "Ecnomus", "Goera", "Hydroptila", "Oecetis", "Psychomyia", "Leptophlebiidae", 
             "Micronecta", "Stenelmis", "Empididae",  "Tabanidae", "Onychogomphus", "Physella")
taxa.Si.common <- c("Gammarus", "Chironomidae", "Corbicula", "Asellidae", "Echinogammarus", "Baetis",
...
comm_UnCont1_contrib <- TDFD_IBC_data$abundance[TDFD_IBC_data$abundance$group == "UnCont1" , colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G1 | colnames(TDFD_IBC_data$abundance) %in% taxa.Si.common]
comm_Cont_contrib <- TDFD_IBC_data$abundance[TDFD_IBC_data$abundance$group == "Cont", colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G3 | colnames(TDFD_IBC_data$abundance) %in% taxa.Si.common]
# Abundance tables to estimate the beta functional diversity
comm_UnCont1_Eq <- TDFD_IBC_data$abundance[TDFD_IBC_data$abundance$group == "UnCont1", colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G1]
comm_Cont_Eq <- TDFD_IBC_data$abundance[TDFD_IBC_data$abundance$group == "Cont", colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G3]
comm_UnCont1_Cont_Eq <- TDFD_IBC_data$abundance[TDFD_IBC_data$abundance$group == "UnCont1" | TDFD_IBC_data$abundance$group == "Cont", colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G1 | colnames(TDFD_IBC_data$abundance) %in% taxa.Si.G3]
...
# Scaling and centering fuzzy data
z.trait_UnCont1_contrib <- scale(trait_eltonian_Eq, center = TRUE, scale = TRUE)
z.trait_Cont_contrib <- scale(trait_eltonian_Eq, center = TRUE, scale = TRUE)
z.trait_UnCont1_Eq <- scale(trait_eltonian_Eq, center = TRUE, scale = TRUE)
z.trait_Cont_Eq <- scale(trait_eltonian_Eq, center = TRUE, scale = TRUE)
````
...
# Step need to compute the contribution and originality of each species to the trait space occupied by their community
...
# Hierarchical clustering form the output of each PCoA
...
# Mean contribution
contrib_mean_UnCont1 <- mean(contrib1_UnCont1$contribution[contrib1_UnCont1$taxa %in% taxa.Si.G1])
contrib_mean_common_UnCont1 <- mean(contrib1_UnCont1$contribution[contrib1_UnCont1$taxa %in% taxa.Si.common])
contrib_mean_UnCont1_perct <- (contrib_mean_UnCont1 * 100) / (contrib_mean_common_UnCont1 + contrib_mean_UnCont1)
contrib_mean_Cont <- mean(contrib1_Cont$contribution[contrib1_Cont$taxa %in% taxa.Si.G3])
contrib_mean_common_Cont <- mean(contrib1_UnCont1$contribution[contrib1_Cont$taxa %in% taxa.Si.common])
...
# Mean originality
orig_mean_UnCont1 <- mean(orig1_UnCont1$orig[orig1_UnCont1$taxa %in% taxa.Si.G1])
orig_mean_common_UnCont1 <- mean(orig1_UnCont1$orig[orig1_UnCont1$taxa %in% taxa.Si.common])
orig_mean_UnCont1_perct <- (orig_mean_UnCont1 * 100) / (orig_mean_common_UnCont1 + orig_mean_UnCont1)
orig_mean_Cont <- mean(orig1_Cont$orig[orig1_Cont$taxa %in% taxa.Si.G3])
orig_mean_common_Cont <- mean(orig1_Cont$orig[orig1_Cont$taxa %in% taxa.Si.common])
...
pathSHP <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/data/QGIS_shp_france" # CHANGE ME to where you want data's file to be load
# Load shape files
...
pv_fit_rq <- summary(fit_rq_shannon, se = "boot")$coefficients["IBCmetals", "Pr(>|t|)"] # p.value = 0.0131
# Plot of the Shannon index against metal contamination gradient
TDFD_IBC_res$TDFD_metrics$color[TDFD_IBC_res$TDFD_metrics$color == "#000000"] <- "#FFFFFF"
par(mar = c(5, 0, 3, 5))
Fig1B <- ggplot(TDFD_IBC_res$TDFD_metrics, aes(x = IBCmetals, y = N1b2)) +
  ...
  annotate("text", 
           x = 0.28, 
           y = 13.5, 
           label = as.character(expression(paste(italic(p), "= 0.0131"))),
           parse = TRUE,
           size = 3, 
           hjust = 0)
```
### Figure 1 - Panel C
```{r, eval = FALSE}
# Test the relationship between Rao's index and metal gradient
...
# Save as png file Fig1 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_Fig1.png"),
     units = "in", width = 10, height = 8, res = 800)
print(Fig1)
dev.off()
```

![Figure 1](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_Fig1.png)

## Figure 2
| <span> |
| :------------------------------------------------------------------------------------------------------------ |
| **NOTE:** This part is based on the scarcity table calculated at the section 10 'Computation of scarcity at local scale' of this tutorial <br/> This figure is composed to 2 panels: <br/> Panel A: Species sorted according to their average values of 1-scarcity in the group of uncontaminated sites (UnCont1) <br/> Panel B: Species sorted according to their average values of 1-scarcity in the group of contaminated sites (Cont)|
| <span> |
```{r, eval = FALSE}
...
for (i in 1:nrow(Si.G1)) {
  for (j in 1:nrow(Si.G3.40perc)) {
    if(Si.G1$species[i] %in% Si.G3.40perc$species[j]) {
      Si.G1$Si.G3[i] <- Si.G3.40perc$Si[j]
    }
  }
}
...
scarcity.g1 <- ggplot(Si.G1, aes(x = Si, y = new.names.species)) + 
  geom_point(size = 2.5) +
  geom_point(data = Si.G1, aes(x = Si.G3, y = new.names.species), color = as.character(Si.G1$Si.G3.color), size = (Si.G1$Si.G3.size + 0.5), shape = Si.G1$Si.G3.shape) +
  theme_bw(base_size = 16) +
  theme(axis.text.y = element_text(color = Si.G1$occurrence.color, size = 15),
...
scarcity.g3 <- ggplot(Si.G3, aes(x = Si, y = new.names.species)) + 
  geom_point((size = 2.5) +
  theme_bw(base_size = 16) +
  theme(axis.text.y = element_text(color = Si.G3$occurrence.color, size = 15),
        axis.text.x = element_text(size = 15),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) +
...
### Figure 2 - Panels A-B
```{r, eval = FALSE}
# Let's glue the different plots together to build Fig.2
design <- c(
  area(1, 1, 1),
  area(1, 2, 1)
)
# Build Fig.2
Fig2 <- scarcity.g1 + scarcity.g3 +
  plot_layout(design = design, widths = c(1, 1), heights = c(1, 1))
# Save as png file Fig.2 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_Fig2.png"),
     units = "in", width = 16, height = 16, res = 800)
print(Fig2)
dev.off()
```
![Figure 2](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_Fig2.png)

## Figure 3 
| <span> |
| :------------------------------------------------------------------------------------------------------------ |
| **NOTE:** This figure is composed to 2 panels: <br/> Panel A: Principal Coordinate Analysis based on Euclidean distance representing the functional space of macroinvertebrates <br/r> Panel B: Functional contribution and functional originamity for dominant species|
| <span> |
### Figure 3 - Panel A
```{r, eval = FALSE}
...
# Harmonization of scarcity tidy table
Si_global <- Si.tidy %>%
...
Si_global <-   Si_global %>%
  ...
  arrange(Si) %>%
  as.data.frame()
# Select taxa specific to UnCont1 with value > to 40th percentile
Si.G1 <- Si_global[Si_global$group.site == "UnCont1", ]
Si.G1$species <- factor(Si.G1$species, levels = Si.G1$species)
Si.G1.40perc <- Si.G1[Si.G1$Si >= quantile(Si.G1$Si, probs = seq(0, 1, 0.1))[5], ]
# Select taxa specific to Cont with scarcity > to 40th percentile
Si.G3 <- Si_global[Si_global$group.site == "Cont", ]
Si.G3$species <- factor(Si.G3$species, levels = Si.G3$species)
Si.G3.40perc <- Si.G3[Si.G3$Si >= quantile(Si.G3$Si, probs = seq(0, 1, 0.1))[5], ]
# Identify specific taxa to Uncont1 and Cont and common to both with scarcity > to 40th percentile
taxa.Si.G1 <- setdiff(Si.G1.40perc$species, Si.G3.40perc$species)
taxa.Si.G3 <- setdiff(Si.G3.40perc$species, Si.G1.40perc$species)
taxa.Si.common <- intersect(Si.G3.40perc$species, Si.G1.40perc$species)
# Add information to the PCoA table need to perform Fig.3
...
pcoa1 <- pcoa1 %>%
  ...
  mutate(id = "cluster") %>%
  mutate(Si.40perc.color = ifelse(pcoa1$species %in% taxa.Si.G1, "#6699FF", 
                                  ifelse(pcoa1$species %in% taxa.Si.G3, "#FF6666", 
                                         ifelse(pcoa1$species %in% taxa.Si.common, "#404040", "#CCCCCC")))) %>%
  mutate(Si.40perc.pch = ifelse(pcoa1$species %in% taxa.Si.G1, 17, 
                                ifelse(pcoa1$species %in% taxa.Si.G3, 17, 
                                       ifelse(pcoa1$species %in% taxa.Si.common, 17, 19)))) 
pcoa1 <- pcoa1 %>%
  mutate(Si.40perc.occurrence = ifelse(pcoa1$Si.40perc.color == "#6699FF", "UnCont1", 
                                       ifelse(pcoa1$Si.40perc.color == "#FF6666", "Cont", 
                                              ifelse(pcoa1$Si.40perc.color == "#404040", "Common", "Rare")))) %>%
  select(species, clust, clust.color, Si.40perc.color, Si.40perc.pch, Si.40perc.occurrence, id, Axis.1, Axis.2)
...
pcoa1$Si.40perc.color <- factor(pcoa1$Si.40perc.color, levels = unique(pcoa1$Si.40perc.color)[c(2, 1, 3, 4)])
pcoa1$Si.40perc.occurrence <- factor(pcoa1$Si.40perc.occurrence, levels = unique(pcoa1$Si.40perc.occurrence)[c(2, 1, 3, 4)])
# Estimation of the convex hull for each functional group according to the first two axis of PCoA
hull_hc <- pcoa1 %>%
  group_by(clust) %>%
  slice(chull(Axis.1, Axis.2))
# Plot of the PCoA with convex hulls
gplot.pcoa <- ggplot(pcoa1, aes(x = Axis.1, y = Axis.2)) +
  geom_point(shape = 19, size = 2, col = pcoa1$Si.40perc.color)
...
legend_occurrence_plot <- ggplot(pcoa1, aes(x = Axis.1, y = Axis.2, color = Si.40perc.occurrence)) +
  geom_point(size = 2) + 
  scale_color_manual(values = levels(pcoa1$Si.40perc.color)) +
  theme_bw(base_size = 14) +
  theme_void() +
  theme(panel.grid.minor = element_blank(),
...
# Build Fig.3A
Fig3B <- ggplot(table_percent, aes(x = group, y = values)) +
  geom_col(aes(fill = type), width = 0.7) +
  geom_text(aes(y = lab_ypos, label = values, group = type), size = 5, colour = ifelse(table_percent$lab_ypos < 49, "white", "black")) +
  scale_fill_manual(values = c("#0066CC", "#FF0000","#404040")) +
  theme_bw(base_size = 14) +
...
### Figure 3 - Panels A-B
```{r, eval = FALSE}
# Let's glue the different plots together to build Fig.3
design <- c(
  area(1, 1, 5, 1),
  area(1, 2, 1, 2),
  area(2, 2, 4, 2),
...
# Build Fig.3
Fig3 <- Fig3A + plot_spacer() + Fig3B + plot_spacer() +
  plot_layout(design = design, widths = c(0.7, 0.3), heights = c(0.5, 0.5))
# Save as png file Fig.3 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures"
png(paste0(pathFig, "/Rplot_trait_x_IBC_Fig3.png"),
     units = "in", width = 10, height = 8, res = 800)
print(Fig3)
dev.off()
```
![Figure 3](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_Fig3.png)
...
summary(mrpp1)
# Save as png file Fig.S1 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS1.png"),
     units = "in", width = 10, height = 8, res = 800)
with(mrpp1, {
 fig.dist <- hist(boot.deltas, xlim = range(c(delta,boot.deltas)), main = "", xlab = "Delta", col = "blue", cex.lab = 1.5, cex.axis = 1.5)
  mtext("MRPP (observed vs expected delta)", line = 2, cex = 1.5)
  mtext("Obsered delta = 8.591, Expected delta = 10.7, p = < 0.0001", line = 1, cex = 1.5)
...
dev.off()
```
![Figure S1](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS1.png)
...
FigS2 <- ggplot(table_tot, aes(x = as.factor(id.modality), y = value)) +
  ...
  ylab("") +
  ylim(0, 1)
# Save as png file Fig.S2 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS2.png"),
     units = "in", width = 10, height = 8, res = 800)
print(FigS2)
dev.off()
```
![Figure S2](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS2.png)
...
# Save as png file Fig.S1 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS1.png"),
     units = "in", width = 10, height = 8, res = 800)
plot(sac2, ci.type = "poly", lwd = 2, ci.lty = 0, ci.col = "lightgray", ylab = "Species", las = 1, cex.lab = 1.5, cex.axis = 1.5)
boxplot(sac2, col = "yellow", add = TRUE)
dev.off()
```
![Figure S3](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS3.png)
...
pcoa2.G1G2G3 <- pcoa2.G1G2G3 %>%
  ...
  mutate(id = "cluster") %>%
select(species, group.site, group.color, clust.color, clust, id, Axis.1, Axis.2)
...
# Plot for UnCont1
my_text1 <- "UnCont1"
my_grob1 <- grid.text(my_text1, x = 0.09, y = 0.05, gp = gpar(col = "black", fontsize = 14))
...
# Plot for UnCont2
my_text2 <- "UnCont2"
my_grob2 <- grid.text(my_text2, x = 0.09, y = 0.05, gp = gpar(col = "black", fontsize = 14))
...
# Plot for Cont
my_text3 <- "Cont"
my_grob3 <- grid.text(my_text3, x = 0.06, y = 0.05, gp = gpar(col = "black", fontsize = 14))
...
my_groball <- grid.text(my_textall, x = 0.255, y = 0.05, gp = gpar(col = "black", fontsize = 14))
...
plot(design)
# Build Fig.S4
FigS4 <- gplot1 + gplot3 + gplot2 + gplotall +
  plot_layout(design = design, widths = c(1, 1, 1, 1), heights = c(1, 1, 1, 1))
# Save as png file Fig.S4 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS4.png"),
     units = "in", width = 12, height = 10, res = 800)
print(FigS4)
dev.off()
```
![Figure S4](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS4.png)
...
# Save as png file Fig.S5 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS5.png"),
     units = "in", width = 12, height = 10, res = 800)
print(FigS5)
dev.off()
```
![Figure S5](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS5.png)
...
# Save as png file Fig.S6 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS6.png"),
     units = "in", width = 10, height = 8, res = 800)
par(mar = c(5, 5, 3, 5))
plot(1, mean(table_FD$FD.FDis[table_FD$group == "UnCont1"]),
     xlim = c(0.5, 3.5), ylim = c(0, (max(table_FD$FD.FDis) + 2)),
     las = 1, xaxt = "n", ann = FALSE, pch = 17, col = "#000000", cex = 2)
axis(1, seq(0, 4, 1), labels = c("", "UnCont1", "UnCont2", "Cont", ""))
points(rep(1, length(table_FD$FD.FDis[table_FD$group == "UnCont1"])), 
       table_FD$FD.FDis[table_FD$group == "UnCont1"], 
       cex = 1, pch = 19, col = "#0066CC")
points(2, mean(table_FD$FD.FDis[table_FD$group == "UnCont2"]),
       pch = 17, col = "#000000", cex = 2)
points(rep(2, length(table_FD$FD.FDis[table_FD$group == "UnCont2"])), 
       table_FD$FD.FDis[table_FD$group == "UnCont2"],
       cex = 1, pch = 19, col = "#00CC00")
points(3, mean(table_FD$FD.FDis[table_FD$group == "Cont"]),
       pch = 17, col = "#000000", cex = 2)
points(rep(3, length(table_FD$FD.FDis[table_FD$group == "Cont"])), 
       table_FD$FD.FDis[table_FD$group == "Cont"],
...
dev.off()
```
![Figure S6](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS6.png)
...
# Save as png file Fig.S7 with a resolution of 800 ppi
pathFig <- "C:/Users/benjamin/Documents/TDFD_IBCmetals/Figures" # CHANGE ME to where you want figure to be saved
png(paste0(pathFig, "/Rplot_trait_x_IBC_FigS7.png"),
     units = "in", width = 10, height = 8, res = 800)
par(mar = c(5, 5, 4, 2))
plot(TDFD_res$RaoQ.CH.scale, TDFD_res$FD.FDis, 
     pch = 19, xlab = "", ylab = "", 
     col = "black", cex.axis = 1.5)
...
text(0.5, 4.95, labels = bquote(R^2 == .(round(summary(lm1)$adj.r.squared, digits = 2))), 
     adj = 0, cex = 1.5)
text(0.5, 4.65, labels = bquote(italic('p')~'=' ~.(summary(lm1)$coefficients[2, 4])), adj = 0, cex = 1.5)
mtext(text = expression("FD"[Q]), side = 1, line = 3, cex = 1.5)
mtext(text = expression("F"[Dis]), side = 2, line = 3, cex= 1.5)
dev.off()
```
![Figure S7](C:/Users/benjamin/Documents/Travail_BA/IRSTEA_AFB48/paper_traits/analysis_trait_x_IBCmet/Figures_vf/Rplot_trait_x_IBC_FigS7.png)
